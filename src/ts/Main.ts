...
export default class Main extends Scene
{
  ...
  public spreadNumber: number
    ...
    public create(): void
    {
      const card = new Card(this, {x: this.width-this.widthCard, y: this.height/2 - this.heightCard/2+this.paddingY, width:this.widthCard, height:this.heightCard}, {naipe: `${Phaser.Math.Between(1, 4)}`, cardNumber: `${Phaser.Math.Between(1, 13)}`, color: this.setColor() }, undefined, false)
      card.setAngle(90)

      this.spreadCards(this.deck1, this.paddingX, this.height-this.heightCard-this.paddingY, (card: Card) => card.flip(CardState.FACE_UP))
      this.spreadCards(this.deck2, this.paddingX, this.paddingY)

      this.time.delayedCall(8000, () => this.deck2[Phaser.Math.Between(0, 4)].flip(CardState.FACE_UP))
    }
