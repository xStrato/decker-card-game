...
export default class Coin extends GameObjects.Group
{
    ...
    public create(): void
    {
        const text = new Text(this.scene, 0, 0, this.text, { color: this.textColor, fontSize: this.fontSize })
        text.setPosition(this.x-(text.width/2), this.y-(text.height/2))

        const coin1 = new Ellipse(this.scene, this.x,this.y, this.width * .1 + text.width*.2, this.height * .1 + text.width*.2, this.color)
